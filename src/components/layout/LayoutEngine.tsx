...
export function LayoutEngine({ config }: LayoutEngineProps) {
  ...
  return (
    <div className="mx-auto max-w-6xl px-3 sm:px-4 md:px-6 lg:px-8 py-6 sm:py-8 md:py-12 space-y-6 sm:space-y-8 md:space-y-10">
      {modules.map((module) => (
        <ModuleRenderer key={module.id} config={module} />
      ))}
    </div>
  )
}
